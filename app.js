...
function card(x,capital,risk){
 ...
 '<div class="confidence"><i style="width:'+Math.min(100,Number(s.score))+'%"></i></div><div class="reason">'+(s.side==="LONG"?"Bullish":"Bearish")+' setup • score '+s.score+' • ATR '+round2(s.atr)+'</div><div class="checks">'+['trend','vwap','momentum','volume','breakout'].map(k=>'<span class="'+(s.checks?.[k]?'check-on':'check-off')+'">'+(s.checks?.[k]?'✓ ':'• ')+k.toUpperCase()+'</span>').join('')+'</div><div class="tiny" style="margin-top:7px">Confirmed: '+((s.reasons||[]).join(', ')||'none')+' • Capital '+money(capital)+' • planned risk '+risk+'% • estimated reward '+money(reward)+' • trading execution is disabled</div></div>';
}
