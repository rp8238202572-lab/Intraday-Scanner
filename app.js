...
async function scan(){
 ...
 try{
   ...
   const breadthItems=signalCalculated.map(x=>x.signal?.diagnostics?.breadthDirection).filter(Boolean);
   const advances=breadthItems.filter(x=>x==="ADVANCE").length;
   const declines=breadthItems.filter(x=>x==="DECLINE").length;
   const unchanged=breadthItems.filter(x=>x==="UNCHANGED").length;
   const breadthTotal=advances+declines+unchanged;
   const breadthLabel=advances>declines?"Positive":declines>advances?"Negative":"Balanced";
   ...
   const labels={SCORE_BELOW_MIN:"Score below minimum",VOLUME_NOT_READY:"Volume data not ready",QUANTITY_ZERO:"Quantity = 0",QUALITY_GATE:"Core + candle/breakout confirmation gate",INSUFFICIENT_CANDLES:"Insufficient candles",INDICATOR_DATA:"Indicator data invalid",INVALID_CANDLES:"Invalid candle data",NO_SETUP:"No setup conditions met",REQUEST_ERROR:"Request / data error"};
