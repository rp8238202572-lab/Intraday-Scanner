...
export function calculateSignal(candles,{capital=2000,riskPct=1,minScore=65}={}){
  ...
  const reasons=[];if(checks.trend)reasons.push("Trend");if(checks.vwap)reasons.push("VWAP");if(checks.momentum)reasons.push("RSI");if(checks.volume)reasons.push("Volume");if(checks.breakout)reasons.push("Breakout");
  ...
  return {signal:side==="LONG"?"BUY_SETUP":"SELL_SETUP",side,score,price,stop:side==="LONG"?price-stopDist:price+stopDist,target:side==="LONG"?price+stopDist*1.8:price-stopDist*1.8,quantity:qty,rsi:rv,vwap:vw,atr:a,volumeRatio:volRatio,checks,reasons,signalCandleStart:Number(completed.at(-1)?.start||completed.at(-1)?.timestamp||0),diagnostics:diag("VALID","All configured signal requirements passed.",diagnosticsBase)};
}
